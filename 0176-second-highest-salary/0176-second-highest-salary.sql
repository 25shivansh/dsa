...
select  (select distinct  salary as SecondHighestSalary from(select salary , dense_rank() over(order by salary desc) as rnk from employee )as e where e.rnk=2) as secondHighestSalary ;
